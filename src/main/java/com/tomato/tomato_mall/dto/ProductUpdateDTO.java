package com.tomato.tomato_mall.dto;

import jakarta.validation.Valid;
import jakarta.validation.constraints.*;
import lombok.Data;

import java.math.BigDecimal;
import java.util.List;

/**
 * 商品更新数据传输对象
 * <p>
 * 该DTO封装了更新商品时所需的信息，用于前端向后端传递更新商品请求的数据。
 * 设计遵循部分更新原则，除商品ID外的其他字段均为可选，仅更新提供的非空字段。
 * 所有字段都通过Bean Validation进行数据验证，确保数据的有效性。
 * </p>
 * <p>
 * 该对象通常用于：
 * 1. 接收前端商品编辑表单提交的数据
 * 2. 在控制器和服务层之间传递商品更新数据
 * 3. 作为商品更新过程的输入参数
 * </p>
 * <p>
 * 商品ID字段是必填的，主要用于标识要更新的商品。
 * </p>
 *
 * @author Team CBDDL
 * @version 1.0
 */
@Data
public class ProductUpdateDTO {
    /**
     * 商品ID
     * <p>
     * 要更新的商品ID，必填字段。
     * 用于标识待更新的商品记录。
     * </p>
     */
    @NotNull(message = "Product ID cannot be null")
    private Long id;

    /**
     * 商品标题
     * <p>
     * 商品的名称或标题，可选字段。
     * 如果提供，长度限制为2到50个字符。
     * </p>
     */
    @Size(min = 2, max = 50, message = "Product title must be between 2 and 50 characters")
    private String title;

    /**
     * 商品价格
     * <p>
     * 商品的销售价格，可选字段。
     * 如果提供，必须大于等于0。
     * </p>
     */
    @DecimalMin(value = "0.00", message = "Product price must be greater than or equal to 0")
    private BigDecimal price;

    /**
     * 商品评分
     * <p>
     * 商品的评分，可选字段。
     * 如果提供，有效范围为0到10。
     * </p>
     */
    @Min(value = 0, message = "Product rate cannot be less than 0")
    @Max(value = 10, message = "Product rate cannot be greater than 10")
    private Double rate;

    /**
     * 商品描述
     * <p>
     * 商品的简要描述，可选字段。
     * 最大长度为255个字符。
     * </p>
     */
    @Size(max = 255, message = "Product description cannot exceed 255 characters")
    private String description;

    /**
     * 商品封面
     * <p>
     * 商品封面的URL地址，可选字段。
     * 最大长度为500个字符。
     * </p>
     */
    @Size(max = 500, message = "Product cover URL cannot exceed 500 characters")
    private String cover;

    /**
     * 商品详情
     * <p>
     * 商品的详细介绍，可选字段。
     * 最大长度为500个字符。
     * </p>
     */
    @Size(max = 500, message = "Product detail cannot exceed 500 characters")
    private String detail;

    /**
     * 商品规格列表
     * <p>
     * 商品的规格信息列表，每个规格包含名称和值。
     * 可选字段，但如果提供，列表中的每个元素都需要验证。
     * 如果提供，将完全替换现有规格。
     * </p>
     */
    @Valid
    private List<SpecificationDTO> specifications;
}